...
Stack<Token>* shunting_yard2(Stack<Token>* input) {
    ...
    while (!input->empty())  //Si no hay Sack de strings no sigo
    {
        ...
        case Token::RIGHT_PAREN:
            ...
            while ((!operatorSTK->empty()))
            {
                if (operatorSTK->top().getType() == Token::LEFT_PAREN) {
                    operatorSTK->pop();
                    break;
                }
                //Mientras el Stack de operadores no sea nulo Y el token no sea un "(" agrega
        ...
        case Token::BINARY_OP:
            ...
            if (operatorSTK->empty() || operatorSTK->top().getAssociativity() == Token::LEFT) {
                operatorSTK->push(actualToken);
                break;
            }
            if ((actualToken.getPriority() > operatorSTK->top().getPriority()) ||
                ((actualToken.getPriority() == operatorSTK->top().getPriority()) && actualToken.getAssociativity() == Token::RIGHT)
                ) {
                operatorSTK->push(actualToken);
                break;
            }
            if ((actualToken.getPriority() < operatorSTK->top().getPriority()) ||
                ((actualToken.getPriority() == operatorSTK->top().getPriority()) && (actualToken.getAssociativity() == Token::LEFT))
                ) {
                while ((actualToken.getPriority() < operatorSTK->top().getPriority()) ||
                    ((actualToken.getPriority() == operatorSTK->top().getPriority()) && actualToken.getAssociativity() == Token::LEFT)
                    ) {
                    //outputSTK->push(actualToken);
                    //actualToken.print();
